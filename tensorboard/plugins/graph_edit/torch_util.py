...
################################################################################
#
# The torch graph is generated by tracing the torch module. It's done through
# overloading the torch tensor symbol and torch module
#
# After generating the graph for an running torch module, its original symbol
# should be reverted
#
################################################################################
class SymbolResolve(object):
...
class TensorSymbol(object):
    __symbol_list__ = []

    @classmethod
    def appendSymbol(cls, symbol_resolve):
        cls.__symbol_list__.append(symbol_resolve)

    @classmethod
    def totalSymbols(cls):
        return len(cls.__symbol_list__);
...
###############################################################################
#
# overloaded symbol for torch tensor and function
#
###############################################################################
class TorchAbsSymbol(SymbolResolve):
    """ resolve the overloaded torch.abs symbol """
    def __init__(self, torch_func):
        super(TorchAbsSymbol, self).__init__(torch_func)

    def resolveSymbol(self):
        torch.abs = self.torch_func

class TorchUnsqueezeSymbol(SymbolResolve):
    """ unsqueeze symbol resolve """
    def __init__(self, torch_func):
        super(TorchUnsqueezeSymbol, self).__init__(torch_func)

    def resolveSymbol(self):
        torch.unsqueeze = self.torch_func

class TorchFlattenSymbol(SymbolResolve):
    def __init__(self, torch_func):
        super(TorchFlattenSymbol, self).__init__(torch_func)

    def resolveSymbol(self):
        torch.flatten  = self.torch_func

class TorchCatSymbol(SymbolResolve):
    def __init__(self, torch_func):
        super(TorchCatSymbol, self).__init__(torch_func)

    def resolveSymbol(self):
        torch.cat = self.torch_func

class TorchReluSymbol(SymbolResolve):
    def __init__(self, torch_func):
        super(TorchReluSymbol, self).__init__(torch_func)

    def resolveSymbol(self):
        torch.relu = self.torch_func

class TorchReluSymbolE(SymbolResolve):
    def __init__(self, torch_func):
        super(TorchReluSymbolE, self).__init__(torch_func)

    def resolveSymbol(self):
        torch.relu_ = self.torch_func

class TorchPaddingSymbol(SymbolResolve):
    def __init__(self, torch_func):
        super(TorchPaddingSymbol, self).__init__(torch_func)

    def resolveSymbol(self):
        F.pad = self.torch_func

class TorchAdaptiveAvgPool2dSymbol(SymbolResolve):
    def __init__(self, torch_func):
        super(TorchAdaptiveAvgPool2dSymbol, self).__init__(torch_func)

    def resolveSymbol(self):
        F.adaptive_avg_pool2d = self.torch_func

class TorchMaxPool2dSymbol(SymbolResolve):
    def __init__(self, torch_func):
        super(TorchMaxPool2dSymbol, self).__init__(torch_func)

    def resolveSymbol(self):
        F.max_pool2d = self.torch_func

class TorchAvgPool2dSymbol(SymbolResolve):
    def __init__(self, torch_func):
        super(TorchAvgPool2dSymbol, self).__init__(torch_func)

    def resolveSymbol(self):
        F.avg_pool2d = self.torch_func

class TorchDropoutSymbol(SymbolResolve):
    def __init__(self, torch_func):
        super(TorchDropoutSymbol, self).__init__(torch_func)
...
def enter_symbol_ctx():
    ...
    class TorchAbs(object):
        """ overload the abs operation for torch tensor """
        def __init__(self, torch_abs):
            self._torch_abs = torch_abs
            TensorSymbol.appendSymbol(TorchAbsSymbol(torch_abs))
    ...
    class TorchUnsqueeze(object):
        """ overload unsqueeze operation for torch tensor """
        def __init__(self, torch_unsqueeze):
            self._torch_unsqueeze = torch_unsqueeze
            TensorSymbol.appendSymbol(TorchUnsqueezeSymbol(torch_unsqueeze))
    ...
    torch.unsqueeze = TorchUnsqueeze(torch.unsqueeze)

    class TorchFlatten(object):
        """ overload flatten operation for torch tensor """
        def __init__(self, torch_flatten):
            self._torch_flatten = torch_flatten
            TensorSymbol.appendSymbol(TorchFlattenSymbol(torch_flatten))
    ...
    class TorchCat(object):
        """ overload the concat operation for torch tensor """
        def __init__(self, torch_cat):
            self._torch_cat = torch_cat
            TensorSymbol.appendSymbol(TorchCatSymbol(torch_cat))
    ...
    class TorchRelu(object):
        """ overload relu operation for torch tensor """
        def __init__(self, torch_relu):
            # no-inplace relu
            self._torch_relu = torch_relu
            TensorSymbol.appendSymbol(TorchReluSymbol(torch_relu))
    ...
    class TorchRelu_(object):
        """ overload relu_ operation for torch tensor """
        def __init__(self, torch_relu_):
            # inplace relu
            self._torch_relu_ = torch_relu_
            TensorSymbol.appendSymbol(TorchReluSymbolE(torch_relu_))
    ...
    class TorchPadding(object):
        """ overload padding operation for torch function """
        def __init__(self, torch_padding):
            self.torch_padding = torch_padding
            TensorSymbol.appendSymbol(TorchPaddingSymbol(torch_padding))
    ...
    class TorchAdaptiveAvgPool2d(object):
        """ overload adaptive average pool2d for torch function """
    ...
    class TorchMaxPool2d(object):
        """ overload max pool2d for torch function """
        def __init__(self, torch_maxpool2d):
            self.torch_maxpool2d = torch_maxpool2d
            TensorSymbol.appendSymbol(TorchMaxPool2dSymbol(torch_maxpool2d))
    ...
    class TorchAvgPool2d(object):
        """ overload average 2d for torch function """
        def __init__(self, torch_avgpool2d):
            self.torch_avgpool2d = torch_avgpool2d
            TensorSymbol.appendSymbol(TorchAvgPool2dSymbol(torch_avgpool2d))
    ...
    class TorchDropout(object):
        """ overload the dropout for torch function """
        def __init__(self, torch_dropout):
            self.torch_dropout = torch_dropout
            TensorSymbol.appendSymbol(TorchDropoutSymbol(torch_dropout))
