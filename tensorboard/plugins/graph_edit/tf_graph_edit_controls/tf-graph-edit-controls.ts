...
// TODO(stephanwlee): Move this to tf-graph-edit-dashboard
...
// TODO(stephanwlee) Move this to tf-graph-edit.html when it becomes TypeScript.
...
Polymer({
  ...
  properties: {
    ...
    saveTypes:{
      type: Array,
      value: ['onnx', 'caffe2', 'torch', 'tf', 'caffe'],
    },
    edgeMode: {
      type: Boolean,
      value: false,
  ...
  _editModelChange: function(edgeMode){
    if(edgeMode){
      return 'EDGE EDIT'
    }
    return 'NODE EDIT'
  ...
  fit: function(): void {
    ...
    // document.querySelector. tf-graph-edit renders tf-graph-edit-scene#scene
  ...
  saveModel: function(){
    ...
    params.set('type', this.saveTypes[this._selectedMode]);
    params.set('path', this.path);
  
    new Promise(() => {
      fetch(tf_backend.getRouter().pluginRoute('graphedit', '/save', params)).then((res) => {
  ...
  _selectedRunIndexChanged: function(runIndex: number): void {
    ...
    // this.$['trace-inputs'].active = false; // Set trace input to off-state.TODO:
