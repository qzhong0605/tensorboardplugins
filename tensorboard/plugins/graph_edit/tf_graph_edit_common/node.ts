...
==============================================================================*/
module tf.graph.edit.scene.node {
  import RenderNodeInfo = tf.graph.edit.render.RenderNodeInfo;
  /**
...
export function getContextMenu(node: Node, sceneElement) {
  let menu = [{
    title: (d): string => {
      return "delete this operater node";
    },
    action: (elm, d, i) => {
      // sceneElement.fire('node-toggle-extract', {name: node.name});
      if(node['op']!=undefined){// addition nodes
        sceneElement.deleteNode(node['name'])
      }
    }
  }];
...
export function getGroupSettingLabel(node: Node) {
  return tf.graph.edit.getGroupSeriesNodeButtonString(
    getContainingSeries(node) !== null ? tf.graph.edit.SeriesGroupingType.GROUP :
...
function getGradient(
  ...
  let escapedId = tf.graph.edit.util.escapeQuerySelector(id);
 ...
 */
export function traceInputs(renderGraphInfo: tf.graph.edit.render.RenderGraphInfo) {
  // Reset all styling.
...
export function _getAllContainedOpNodes(
    nodeName: string, renderGraphInfo: tf.graph.edit.render.RenderGraphInfo) {
  let opNodes = [];

  // Get current node.
  let node = renderGraphInfo.getNodeByName(nodeName) as tf.graph.edit.GroupNode |
      tf.graph.edit.OpNode;

  // If node is already OpNode then return the node plus its input embeddings.
  if (node instanceof tf.graph.edit.OpNodeImpl) {
    return [node].concat(node.inEmbeddings);
  }

  // Otherwise, make recursive call for each node contained by the GroupNode.
  let childNodeNames = (node as tf.graph.edit.GroupNode).metagraph.nodes();
...
export function traceAllInputsOfOpNode(
    renderGraphInfo: tf.graph.edit.render.RenderGraphInfo, startNode: OpNode,
  ...
  _.each(inputs, function(nodeInstance) {
    ...
    if (resolvedNode instanceof MetanodeImpl) {
      let resolvedNodeName = tf.graph.edit.getStrictName(resolvedNode.name);
  ...
  for (let index = 1; currentNode.name !== tf.graph.edit.ROOT_NAME; index++) {
...
function _markParentsOfNodes(visibleNodes: {[nodeName: string]: Node}) {
  _.forOwn(visibleNodes, function(nodeInstance: Node) {
    ...
    while (currentNode.name !== tf.graph.edit.ROOT_NAME) {
...
export function getVisibleParent(
    renderGraphInfo: tf.graph.edit.render.RenderGraphInfo,
    currentNode: tf.graph.edit.Node) {
  let found = false;
  ...
  while (!found) {
    ...
    if (currentParent === undefined) {
      ...
      if (renderNode &&
          (renderNode.expanded || currentParent instanceof graph.edit.OpNodeImpl)) {
        found = true;
      }
    }
  }  // Close while loop.
