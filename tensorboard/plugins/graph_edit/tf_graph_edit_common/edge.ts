...
==============================================================================*/
module tf.graph.edit.scene.edge {
 ...
 * @param graph
 * @param sceneElement <tf-graph-edit-scene> polymer element.
          ...
          edgeGroup
              ...
              .on('contextmenu',(d, i) => {
                ...
                let contextMenuFunction = contextmenu.getMenu(
                  sceneElement, getContextMenu(sceneElement,d, edgeGroup));
...
/**
 * Returns the d3 context menu specification for the provided edge.
 */
export function getContextMenu(sceneElement, edge, edgeGroup) {
  let menu = [{
    title: (d): string => {
      return "delete this edge";
    },
    action: (elm, d, i) => {
      sceneElement.deleteEdge(edge, edgeGroup)
    }
  }];
...
function getPathSegmentIndexAtLength(
  ...
  const path = document.createElementNS(tf.graph.edit.scene.SVG_NAMESPACE, 'path');
