...
export class OpNodeImpl implements OpNode {
   ...
   */
  constructor(rawNode: tf.graph.edit.proto.NodeDef) {
    this.op = rawNode.op;
...
export function joinStatsInfoWithGraph(
    graph: SlimGraph, stats: tf.graph.edit.proto.StepStats,
...
export function build(
    graphDef: tf.graph.edit.proto.GraphDef, params: BuildParams,
      ...
      .runAsyncTask(
          ...
          () => {
            ...
            const processRawNode = rawNode => {
              ...
              if(rawNode.include==undefined){
                if(opNode.attr!=undefined){
                  opNode.attr.forEach(function(d){
                    if(d.key != "shape"){
                      var t = d.value;
                      while(typeof(t)=='object'){
                        var str = Object.keys(t)[0];
                        t = t[str];
                      }
                      d.value = t;
                    }else{
            ...
            const processFunction = (func: tf.graph.edit.proto.FunctionDef) => {
...
export function getIncludeNodeButtonString(include: InclusionType) {
  if (include === tf.graph.edit.InclusionType.EXCLUDE) {
...
export function getGroupSeriesNodeButtonString(group: SeriesGroupingType) {
  if (group === tf.graph.edit.SeriesGroupingType.GROUP) {
...
export function toggleNodeSeriesGroup(
  map: { [name: string]: tf.graph.edit.SeriesGroupingType }, name: string) {
  if (!(name in map) || map[name] === tf.graph.edit.SeriesGroupingType.GROUP) {
    map[name] = tf.graph.edit.SeriesGroupingType.UNGROUP;
  } else {
    map[name] = tf.graph.edit.SeriesGroupingType.GROUP;
  }
};

} // close module tf.graph.edit
