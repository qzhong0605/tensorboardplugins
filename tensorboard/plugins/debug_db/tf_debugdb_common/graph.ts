...
export class OpNodeImpl implements OpNode {
   ...
   */
  constructor(rawNode: tf.debug.proto.NodeDef) {
    this.op = rawNode.op;
...
export function joinStatsInfoWithGraph(
    graph: SlimGraph, stats: tf.debug.proto.StepStats,
...
export function build(
    graphDef: tf.debug.proto.GraphDef, params: BuildParams,
  ...
  return tf.debug.util
      .runAsyncTask(
          ...
          () => {
            ...
            const processFunction = (func: tf.debug.proto.FunctionDef) => {
...
export function getIncludeNodeButtonString(include: InclusionType) {
  if (include === tf.debug.InclusionType.EXCLUDE) {
...
export function getGroupSeriesNodeButtonString(group: SeriesGroupingType) {
  if (group === tf.debug.SeriesGroupingType.GROUP) {
...
export function toggleNodeSeriesGroup(
  map: { [name: string]: tf.debug.SeriesGroupingType }, name: string) {
  if (!(name in map) || map[name] === tf.debug.SeriesGroupingType.GROUP) {
    map[name] = tf.debug.SeriesGroupingType.UNGROUP;
  } else {
    map[name] = tf.debug.SeriesGroupingType.GROUP;
  }
};

} // close module tf.debug
