...
export interface HierarchyParams {
  ...
  seriesMap: { [name: string]: tf.convert.SeriesGroupingType };
 ...
 */
export function build(graph: tf.convert.SlimGraph, params: HierarchyParams,
  ...
  return tf.convert.util
      .runAsyncTask(
          ...
          tracker)
      .then(() => {
        return tf.convert.util.runAsyncTask('Detect series', 20, () => {
      ...
      .then(() => {
        return tf.convert.util.runAsyncTask('Adding edges', 30, () => {
...
export function joinAndAggregateStats(
    h: Hierarchy, stats: tf.convert.proto.StepStats) {
  // Get all the possible device and XLA cluster names.
  let deviceNames = {};
  let xlaClusterNames = {};
      ...
      if (!opNode.compatible) {
        if (opNode.owningSeries) {
          if (hierarchyParams &&
              ...
              === tf.convert.SeriesGroupingType.UNGROUP) {
  ...
  _.each(graph.nodes, (node, nodeName) => {
    ...
    for (let i = 0; i < path.length; i++) {
      ...
      if (!child) {
        ...
        if (name.indexOf(tf.convert.FUNCTION_LIBRARY_NODE_PREFIX) === 0 &&
            parent.name === tf.convert.ROOT_NAME) {
...
function groupSeries(metanode: Metanode, hierarchy: Hierarchy,
    ...
    map: { [name: string]: tf.convert.SeriesGroupingType },
    useGeneralizedSeriesPatterns: boolean) {
  let metagraph = metanode.metagraph;
  _.each(metagraph.nodes(), n => {
    let child = metagraph.node(n);
    if (child.type === tf.convert.NodeType.META) {
      groupSeries(
          <Metanode>child,
          hierarchy,
          seriesNames,
  ...
  _.each(seriesDict, function(seriesNode: SeriesNode, seriesName: string) {
    ...
    if (nodeMemberNames.length < threshold && !(seriesNode.name in map)) {
      map[seriesNode.name] = tf.convert.SeriesGroupingType.UNGROUP;
    }
...
} // close module tf.convert.hierarchy
