...
export function fetchAndParseMetadata(path: string, tracker: ProgressTracker) {
  return tf.convert.util
      ...
      .then((arrayBuffer: ArrayBuffer) => {
        return tf.convert.util.runAsyncPromiseTask(
...
export function fetchAndParseGraphData(path: string, pbTxtFile: Blob,
  ...
  return tf.convert.util
      .runAsyncPromiseTask(
          ...
          tracker)
      .then((arrayBuffer: ArrayBuffer) => {
        return tf.convert.util.runAsyncPromiseTask('Parsing graph.pbtxt', 60, () => {
...
export function parseGraphPbTxt(input: ArrayBuffer):
    Promise<tf.convert.proto.GraphDef> {
...
export function parseStatsPbTxt(input: ArrayBuffer):
    Promise<tf.convert.proto.StepStats> {
...
} // Close module tf.convert.parser.
