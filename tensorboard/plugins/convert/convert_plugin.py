...
class ConvertPlugin(base_plugin.TBPlugin):
  ...
  def __init__(self, context):
    ...
    self._dst_type = None
    # caffe2
    self.src_predict_net = None
    self.src_init_net = None
    # other models
  ...
  def load_model(self, request):
    ...
    if self._src_type == "torch":
      self.input_tensor_size = request.args.get("input_tensor_size").replace(' ', '')
      ...
      if self.model_file in ['inception_v3', 'googlenet']:
        self._src_tb_graph = torch_util.freeze_graph(globals().get(self.model_file)(pretrained=True, aux_logits=False ,transform_input=False),
                                                   torch.randn(tuple(map(int, tensor_size_list))))
      else:
        self._src_tb_graph = torch_util.freeze_graph(globals().get(self.model_file)(pretrained=True),
                                                     torch.randn(tuple(map(int, tensor_size_list))))
    elif self._src_type == "caffe2":
      self.src_predict_net = request.args.get("predict_net")
      self.src_init_net = request.args.get("init_net")
      self.input_tensor_size = request.args.get("input_tensor_size")
      if not (os.path.exists(self.src_predict_net) and os.path.exists(self.src_init_net)):
        # send a response to frontend and report that model file doesnot exist
        pass
      self._src_tb_graph = c2graph_util.C2Graph(self.src_predict_net, self.src_init_net, "pb")
